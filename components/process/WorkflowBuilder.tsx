...
const stageLibrary: Array<{ id: string; name: string; description: string }> = [
...
export function WorkflowBuilder({
  ...
  jobPostTitle = 'Recruitment Process',
}: WorkflowBuilderProps) {
  const [workflowStages, setWorkflowStages] = useState<ProcessStage[]>(initialStages)
  const [draggedStage, setDraggedStage] = useState<{ id: string; name: string; description: string } | null>(null)
  const [isSaving, setIsSaving] = useState(false)

  const handleDragStart = (stage: { id: string; name: string; description: string }) => {
  ...
  const handleDrop = (e: React.DragEvent, index?: number) => {
    ...
    const stageExists = workflowStages.find((s) => s.name === draggedStage.name)
    if (stageExists) {
    ...
    const newStage: ProcessStage = {
      ...
      order: index !== undefined ? index : workflowStages.length,
      interviewer_ids: [],
    }
    ...
    if (index !== undefined) {
      ...
      setWorkflowStages(newStages.map((s, i) => ({ ...s, order: i })))
    } else {
      setWorkflowStages([...workflowStages, newStage].map((s, i) => ({ ...s, order: i })))
    }
    setDraggedStage(null)
  }

  const removeStage = (id: string) => {
    setWorkflowStages(workflowStages.filter((s) => s.id !== id).map((s, i) => ({ ...s, order: i })))
  }

  const handleSave = async () => {
    setIsSaving(true)
    try {
      await onSave(workflowStages)
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-500" style={{ fontFamily: 'Noto Sans KR, sans-serif' }}>
                              {stage.interviewer_ids?.length || 0}명의 면접관
                            </div>
                          </div>
                        </div>
                        <button
            ...
            <div className="mt-6 flex gap-3">
              <Button
                onClick={() => {
                  const newStage: ProcessStage = {
                    id: crypto.randomUUID(),
                    name: `새 단계 ${workflowStages.length + 1}`,
        ...
        <div className="w-80 bg-white border-l p-6 overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-2xl" style={{ fontFamily: 'Roboto, sans-serif' }}>
                {workflowStages.length * 3}-{workflowStages.length * 5} days
              </div>
            </div>
