...
- 외부(비가입) 면접관 이메일(`schedules.external_interviewer_emails`)이 포함된 경우에도, **해당 이메일 캘린더의 바쁨을 함께 조회**하여 충돌 계산에 포함합니다.
  - 전제: 자동화를 실행하는 채용담당자(Organizer) 구글 계정이 외부 면접관 캘린더를 조회할 수 있어야 합니다(공유/권한).
  - 정책: 외부 면접관 캘린더를 조회할 수 없으면(권한/공유 문제 등) **자동화는 중단**되며, 공유/가입/연동을 안내합니다.
