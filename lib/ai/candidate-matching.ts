...
import { requireServerGeminiApiKey } from '@/lib/ai/gemini-env';
...
export async function analyzeCandidateMatch(
  ...
  try {
    ...
    const apiKey = requireServerGeminiApiKey();
    ...
    if (!hasResumeFile) {
      ...
      await updateCandidatesResilient(supabase, candidateId, {
    ...
    await updateCandidatesResilient(supabase, candidateId, { ai_analysis_status: 'processing' });
...
반드시 유효한 JSON 형식으로만 응답해주세요. 다른 설명이나 마크다운 없이 순수 JSON만 반환해주세요.`;
    ...
    // 9. DB에 결과 저장 (스키마 캐시에 없는 컬럼은 자동 제외 후 재시도)
    const { error: updateError } = await updateCandidatesResilient(supabase, candidateId, {
      ai_score: score,
      ai_summary: analysisResult.summary,
      ai_strengths: analysisResult.strengths.slice(0, 5),
      ai_weaknesses: analysisResult.weaknesses.slice(0, 5),
    ...
    if (updateError) {
      ...
      console.error(
        '\n⚠️  [analyzeCandidateMatch] 일부 AI 컬럼을 저장하지 못했습니다. 마이그레이션·스키마 캐시를 확인하세요.',
      );
      ...
      console.error('   Supabase > Settings > API > Refresh Schema Cache\n');
      throw new Error(`분석 결과 저장 실패: ${updateError.message ?? '알 수 없는 오류'}`);
    }
    ...
    try {
      const supabase = createServiceClient();
      const { error: failUpdateErr } = await updateCandidatesResilient(supabase, candidateId, {
        ai_analysis_status: 'failed',
        ai_summary: failureSummary,
      });
      if (failUpdateErr) {
        console.error('[analyzeCandidateMatch] 상태 업데이트 실패:', failUpdateErr);
      }
    } catch (updateError) {
...
function formatAnalysisFailureSummary(error: unknown): string {
  ...
  if (
    msg.includes('GOOGLE_GEMINI_API_KEY') ||
    msg.includes('GEMINI_API_KEY') ||
    msg.includes('서버에 없거나')
  ) {
    return 'AI 분석을 실행할 수 없습니다. 배포 서버와 로컬 모두에 GOOGLE_GEMINI_API_KEY(또는 GEMINI_API_KEY)를 넣은 뒤 재배포·재시작하세요. 키는 Google AI Studio(https://aistudio.google.com/apikey)에서 발급할 수 있습니다.';
  }
