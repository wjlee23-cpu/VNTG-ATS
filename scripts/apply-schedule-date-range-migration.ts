/**
 * 면접 일정 날짜 범위 필드 마이그레이션 적용
 * schedules 테이블에 original_start_date, original_end_date, retry_count 컬럼 추가
 */
...
async function executeSQL() {
  ...
  const migrationFile = resolve(
    process.cwd(),
    'supabase/migrations/20260223000001_add_schedule_date_range_fields.sql'
  );
  ...
  if (urlMatch) {
    ...
    try {
      ...
      // 참고: Management API는 Access Token이 필요하지만, Service Role Key로도 시도해봅니다
      ...
      if (response.ok) {
        ...
        console.log(`   에러: ${errorText.substring(0, 200)}\n`);
      }
    } catch (error: any) {
  ...
  // 방법 3: Supabase RPC 함수 시도
  console.log('📝 방법 3: Supabase RPC 함수 시도...\n');
